        ...
        <section className="pt-32 pb-20 lg:pt-40 lg:pb-28 bg-gradient-to-b from-slate-50 via-white to-white">
          <div className="mx-auto max-w-[980px] px-6 sm:px-10 lg:px-16">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-3xl mx-auto text-center">
              ...
              <p className="mt-5 text-[17px] leading-[1.7] text-[#424245] max-w-2xl mx-auto">{subtitle}</p>
              <a href="/start" className="group inline-flex items-center gap-2 mt-8 rounded-xl bg-gradient-to-r from-[#0071e3] to-[#0066cc] px-7 py-3.5 text-[15px] font-bold text-white shadow-lg shadow-[#0071e3]/25 hover:-translate-y-0.5 transition-all duration-200">
        ...
        <section className="py-20 lg:py-28">
          <div className="mx-auto max-w-[980px] px-6 sm:px-10 lg:px-16">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {pains.map((p) => (
                <Section key={p.title} className="rounded-2xl bg-white border border-[#d2d2d7]/80 p-6 hover:shadow-md transition-shadow">
                  <h3 className="text-[15px] font-bold text-[#1d1d1f]">{p.title}</h3>
                  <p className="mt-2 text-[14px] text-[#424245] leading-[1.65]">{p.description}</p>
                </Section>
              ))}
            </div>
          </div>
          ...
          <div className="mx-auto max-w-[980px] px-6 sm:px-10 lg:px-16">
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              {solutions.map((s) => {
                const Icon = s.icon;
                return (
                  <Section key={s.title} className="rounded-2xl bg-white border border-[#d2d2d7]/80 p-7 hover:shadow-md transition-shadow">
                    <div className="w-11 h-11 rounded-xl bg-[#0071e3]/5 flex items-center justify-center text-[#0071e3] mb-4">
                      <Icon size={20} />
                    </div>
                    <h3 className="text-[16px] font-bold text-[#1d1d1f]">{s.title}</h3>
                    <p className="mt-2 text-[14px] text-[#424245] leading-[1.65]">{s.description}</p>
                  </Section>
                );
              })}
            </div>
        ...
        <section className="py-20 lg:py-28">
          <div className="mx-auto max-w-[980px] px-6 sm:px-10 lg:px-16">
            ...
            <div className="space-y-4 max-w-3xl mx-auto">
              {workflows.map((w) => (
                <Section key={w.label} className="rounded-2xl bg-white border border-[#d2d2d7]/80 p-5 sm:p-6">
                  <p className="text-[12px] font-bold text-[#86868b] uppercase tracking-wider mb-3">{w.label}</p>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="rounded-xl bg-[#f5f5f7] border border-[#e8e8ed] p-4">
                      <p className="text-[10px] font-bold text-[#86868b] uppercase">Before</p>
                      <p className="mt-1 text-[15px] font-bold text-[#333336]">{w.before}</p>
                    </div>
                    <div className="rounded-xl bg-[#30D158]/5 border border-emerald/10 p-4">
                      <p className="text-[10px] font-bold text-[#30D158] uppercase">After</p>
                      <p className="mt-1 text-[15px] font-bold text-[#30D158]">{w.after}</p>
                    </div>
                  </div>
                </Section>
              ))}
        ...
        <section className="py-20 lg:py-28">
          <div className="mx-auto max-w-[980px] px-6 sm:px-10 lg:px-16">
            <Section className="text-center max-w-2xl mx-auto">
              ...
              <p className="mt-4 text-[16px] text-[#424245]">Your first workflow is free. Answer a few questions to get started.</p>
              <a href="/start" className="group inline-flex items-center gap-2 mt-8 rounded-xl bg-gradient-to-r from-[#0071e3] to-[#0066cc] px-7 py-3.5 text-[15px] font-bold text-white shadow-lg shadow-[#0071e3]/25 hover:-translate-y-0.5 transition-all duration-200">
