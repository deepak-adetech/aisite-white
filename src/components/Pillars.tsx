          ...
          {pillars.map((p, i) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-2xl bg-white border border-slate-200/80 p-7 hover:shadow-[0_8px_30px_rgba(0,0,0,0.04)] transition-all duration-300"
              >
                <div className="flex items-center justify-between mb-6">
                  <div className={`w-11 h-11 rounded-xl bg-slate-50 flex items-center justify-center ${p.color}`}>
                    <Icon size={20} />
                  </div>
                ...
                <p className="mt-2.5 text-[14px] text-slate-600 leading-[1.65]">{p.description}</p>
              </motion.div>
            );
          })}
        </div>
