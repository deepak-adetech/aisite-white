          ...
          {tiers.map((t, i) => (
            <motion.div
              ...
              className={`rounded-2xl border p-7 flex flex-col ${
                t.featured
                  ? "bg-white border-brand shadow-xl shadow-brand/[0.08] ring-1 ring-brand/10 relative"
                  : "bg-white border-slate-200/80"
              }`}
            >
              {t.featured && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-brand to-brand-dark px-4 py-1 text-[10px] font-bold text-white shadow-md shadow-brand/20">
                  <Sparkles size={11} />
                  Most popular
                </div>
              )}

              <h3 className="text-[16px] font-bold text-slate-900">{t.name}</h3>
              <div className="mt-4 flex items-baseline gap-1.5">
                <span className="text-[2.2rem] font-extrabold tracking-tight text-slate-900">{t.price}</span>
                <span className="text-[14px] text-slate-500 font-medium">/ {t.period}</span>
              </div>
              <p className="mt-3 text-[14px] text-slate-600 leading-[1.6]">{t.description}</p>
              ...
              <ul className="mt-6 space-y-3 flex-1">
                {t.features.map((f) => (
                  <li key={f} className="flex items-start gap-3">
                    ...
                    <span className="text-[14px] text-slate-600">{f}</span>
                  </li>
                ))}
              </ul>
              ...
              <a
                ...
                className={`mt-7 group flex items-center justify-center gap-2 rounded-xl px-6 py-3 text-[14px] font-bold transition-all duration-200 ${
                  t.featured
                    ? "bg-gradient-to-r from-brand to-brand-dark text-white shadow-lg shadow-brand/20 hover:shadow-xl hover:shadow-brand/25 hover:-translate-y-0.5"
